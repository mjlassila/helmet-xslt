...
-->
	...
	<xsl:template name="marcRecord">
		...
		<!-- HELMET-modifications 1.0 mjlassila 20100615 added support for persistent urls to Helmet catalogue-->

		<!-- Tag 035 (System Control Number)  subfield $a contains a code which can be used to create links to Helmet catalogue-->

		<xsl:for-each select="marc:datafield[@tag=856 or @tag=035][@ind2!=2][marc:subfield[@code='u'or @code='a' ]]">
			<location>
			  <!-- Check if we are processing an electronic resource with primary link or other resource which has a persistent URL only -->
			  <xsl:if test="marc:datafield[@tag=856]">
				...
				<!-- End of checking electronic resource -->

				<!-- Persistent URL to Helmet catalogue -->
				<xsl:if test="marc:subfield[@code='a']">
				  <xsl:variable name="recordcode" select="substring-after(marc:subfield[@code='a'],'(FI-HELMET)')"/>
				  <url displayLabel="Permalink">http://www.helmet.fi/record=<xsl:value-of select="$recordcode"/>~S9*fin</url>
				</xsl:if>
			</location>
		</xsl:for-each>
		...
		<recordInfo>
			...
			<!-- 1.44 ntra 2009 -->
			<recordOrigin>Converted from MARCXML to MODS version 3.3 using MARC21slim2MODS3-3-with-HELMET-modifications.xsl
				(Revision 1.00)</recordOrigin>
